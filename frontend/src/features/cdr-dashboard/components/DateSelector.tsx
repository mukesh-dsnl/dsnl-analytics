...
interface DateSelectorProps {
  ...
  status: CdrStatus;
  /**
   * One day, or a From–To range. The service tabs each look at a single day;
   * only Custom opens the range up.
   */
  mode: 'single' | 'range';
}

/**
 * The date control, sized to sit on the same line as the service tabs.
 *
 * It belongs beside them rather than in a filter panel because it is not an
 * ordinary filter: it decides which daily export files the backend opens, so
 * it applies to every tab and sets what the query costs as much as what it
 * returns. That is also why the range form is confined to Custom — a wide
 * range over the busier months is the one thing that makes the page slow.
 */
export function DateSelector({ value, onChange, status, mode }: DateSelectorProps) {
  const min = status.cdr.date_min;
  const max = status.cdr.date_max;

  /** In single mode both ends move together, so the range is always one day. */
  const setDay = (raw: string) => {
    if (!raw) return;
    const day = clampDate(raw, min, max);
    onChange({ ...value, date_from: day, date_to: day });
  };

  /**
   * In range mode, moving one end drags the other along when the range would
   * otherwise end up backwards — picking a "from" after the current "to" means
   * you want that day, not an error message.
   */
  ...
  return (
    <div className="flex items-center gap-2 px-3 h-[46px] rounded-md border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-[#09090B] transition-colors duration-300">
      ...
      {mode === 'single' ? (
        <>
          <label htmlFor="cdr-date" className="sr-only">
            Date
          </label>
          <input
            id="cdr-date"
